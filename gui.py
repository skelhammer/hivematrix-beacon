...
if not app.debug: # More detailed logging when not in debug mode
...
def parse_datetime_utc(dt_str):
    ...
    try:
        # Handle various ISO 8601 formats, including those with 'Z' or timezone offset
...
def get_fr_sla_details_for_open_ticket(target_due_dt, critical_threshold_hours, warning_threshold_hours):
    ...
    if not target_due_dt:
        return "No FR Due Date", "sla-none", float('inf') - 1000 # Ensure sort key is high for no due date
    now_utc = datetime.datetime.now(datetime.timezone.utc)
    time_diff_seconds = (target_due_dt - now_utc).total_seconds()
    hours_remaining_for_status = time_diff_seconds / 3600.0

    if abs(time_diff_seconds) >= (2 * 24 * 60 * 60): # More than 2 days
        unit, formatted_value = "days", f"{hours_remaining_for_status / 24.0:.1f}"
    elif abs(time_diff_seconds) >= (60 * 60): # More than 1 hour
        unit, formatted_value = "hours", f"{hours_remaining_for_status:.1f}"
    elif abs(time_diff_seconds) >= 60: # More than 1 minute
...
def time_since(dt_object, default="N/A"):
    ...
    if days < 0: return "in the future" # Should not happen with valid data
    ...
    return "in the future" # Fallback for negative seconds if days somehow 0
...
def days_since(dt_object, default="N/A"):
    ...
    if diff_days < 0: return "Future Date" # Should not happen
...
def load_and_process_tickets():
    ...
    for filename in os.listdir(TICKETS_DIR):
        if filename.endswith(".txt") and filename[:-4].isdigit():
            ...
            try:
                ...
                # Prepare ticket object for processing and API response
                ticket = {
                    ...
                    'fr_due_by_str': data.get('fr_due_by'), # ISO string
                    'updated_at_str': data.get('updated_at'), # ISO string
                    'created_at_str': data.get('created_at'), # ISO string
                    'type': data.get('type', 'N/A'),
                    'stats': data.get('stats', {}) # Contains first_responded_at, agent_responded_at etc.
                }

                # Add ISO formatted first_responded_at for JS client
                first_responded_at_val = ticket['stats'].get('first_responded_at')
                ticket['first_responded_at_iso'] = first_responded_at_val if first_responded_at_val else None
                ...
                # Parse datetimes for internal logic (sorting, calculations)
                ...
                # Use the string from stats for first_responded_at_dt for consistency
                ticket['first_responded_at_dt'] = parse_datetime_utc(ticket['first_responded_at_iso'])
                ticket['agent_responded_at_dt'] = parse_datetime_utc(ticket['stats'].get('agent_responded_at'))
                # ticket['status_updated_at_dt'] = parse_datetime_utc(ticket['stats'].get('status_updated_at')) # Not currently used
                ...
                ticket['agent_responded_friendly'] = time_since(ticket['agent_responded_at_dt']) # For specific contexts
                ...
                ticket['status_text'] = current_status_text
                ...
                if ticket['status_raw'] == OPEN_TICKET_STATUS_ID:
                    if ticket['first_responded_at_dt'] is None: # FR not met
                        ...
                        # Sort by: 0 for FR pending, then by FR SLA hours remaining (asc), then by last update (desc)
                        ticket['action_sort_key_tuple'] = (0, fr_sla_sort_key, -ticket_updated_timestamp)
                    else: # FR met
                        ticket['sla_text'] = f"Open ({ticket['updated_friendly']})"
                        ticket['sla_class'] = "sla-responded" # Or "sla-in-progress" if preferred
                        # Sort by: 1 for FR met, then by last update (desc)
                    ...
                    if ticket['status_raw'] == WAITING_ON_CUSTOMER_STATUS_ID:
                        ...
                        ticket['sla_class'] = "sla-none" # Or specific "sla-on-hold"
                    elif ticket['status_raw'] == PENDING_TICKET_STATUS_ID:
                        ticket['sla_text'] = f"Pending ({ticket['updated_friendly']})"
                        ticket['sla_class'] = "sla-in-progress" # Or specific "sla-pending"
                    # Other statuses will use the default sla_text and sla_class set earlier.
    ...
    list_waiting_on_agent.sort(key=lambda t: t.get('action_sort_key', float('inf'))) # Oldest updated first
    list_section3_candidates.sort(key=lambda t: t.get('action_sort_key', float('inf'))) # Oldest updated first

    # Remove datetime objects before returning for JSON serialization if they are not needed by client
    # String versions (_str, _iso, _friendly, _days_old) are already there.
    for ticket_list in [list_status_open_tickets, list_waiting_on_agent, list_section3_candidates]:
        for ticket_data in ticket_list:
            ticket_data.pop('fr_due_by_dt', None)
            ticket_data.pop('updated_at_dt', None)
            ticket_data.pop('created_at_dt', None)
            ticket_data.pop('first_responded_at_dt', None)
            ticket_data.pop('agent_responded_at_dt', None)
            ticket_data.pop('action_sort_key_tuple', None) # Remove sort keys
            ticket_data.pop('action_sort_key', None)      # Remove sort keys
...
def dashboard():
    ...
    # dashboard_display_time_utc = generated_time_utc.strftime('%Y-%m-%d %H:%M:%S %Z') # No longer used directly in template
    dashboard_generated_time_iso = generated_time_utc.isoformat() # For JS to potentially use
    ...
    return render_template(INDEX,
                            ...
                            # dashboard_generated_time=dashboard_display_time_utc, # Removed from template
                            dashboard_generated_time_iso=dashboard_generated_time_iso, # For JS if needed
                            auto_refresh_ms=AUTO_REFRESH_INTERVAL_SECONDS * 1000,
                            freshservice_base_url=f"https://{FRESHSERVICE_DOMAIN}/a/tickets/",
                            OPEN_TICKET_STATUS_ID=OPEN_TICKET_STATUS_ID, # For JS if needed for logic
                            WAITING_ON_CUSTOMER_STATUS_ID=WAITING_ON_CUSTOMER_STATUS_ID, # For JS
                            WAITING_ON_AGENT_STATUS_ID=WAITING_ON_AGENT) # For JS

@app.route('/api/tickets')
def api_tickets():
    app.logger.debug("API: /api/tickets called")
    s1_open_tickets, s2_waiting_agent_tickets, s3_remaining_tickets = load_and_process_tickets()

    # Datetime objects are removed by load_and_process_tickets now.
    # Ensure all data being sent is JSON serializable.
    # The 'ticket' objects should now only contain serializable types.

    response_data = {
        's1_open_tickets': s1_open_tickets,
        's2_waiting_agent_tickets': s2_waiting_agent_tickets,
        's3_remaining_tickets': s3_remaining_tickets,
        'total_active_tickets': len(s1_open_tickets) + len(s2_waiting_agent_tickets) + len(s3_remaining_tickets),
        'dashboard_generated_time_iso': datetime.datetime.now(datetime.timezone.utc).isoformat()
    }
    app.logger.debug(f"API: Returning {response_data['total_active_tickets']} total tickets.")
...
if __name__ == '__main__':
    ...
    for file_name in sensitive_files_in_root:
        ...
        if os.path.exists(static_file_path):
            ...
            # else: # AGENTS_FILE, REQUESTERS_FILE, SSL_CERT_FILE are expected in root for loading
                # app.logger.info(f"'{file_name}' found in the application root '{app_root_file_path}'. This is expected for loading data/certs.") # Already logged by load functions
    ...
    try:
        ...
        if "Permission denied" in str(e) or "Errno 13" in str(e) and port == 443:
